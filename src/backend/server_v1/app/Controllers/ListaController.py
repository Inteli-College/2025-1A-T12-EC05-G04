...
class ListaController:
    ...
    def createLista(self, request_nova_lista):
        try:
            nova_lista = lista_schema.load(request_nova_lista, session=db.session)
    ...
    def createListaAndMontagem(self, nova_lista):
        lista_criada, status_code = self.createLista(nova_lista)
        if status_code != 201:
    ...
    def getByIdLista(self, lista_id):
        try:
            byIdLista = Lista.query.get_or_404(lista_id)
